class HospedesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @hospede.update(hospede_params)
        format.html { redirect_to hospede_url(@hospede), notice: "Hospede atualizado com sucesso." }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to hospedes_url, notice: "Hospede deletado com sucesso." }
  ...
  private
    ...
    def hospede_params
      params.require(:hospede).permit(:nome, :idade, :CPF)
    end
end
